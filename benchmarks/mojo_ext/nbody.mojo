# N-body benchmark as a Python-callable Mojo extension module.
# Exposes two integration patterns:
#   1. compute(n)             - PythonObject boundary ("python objects")
#   2. Sim().run(n)/advance() - native Mojo struct holding simulation state
#                                exposed to Python ("native objects")
# Built with: mojo build --emit shared-lib nbody.mojo -o nbody.so
...
def run_nbody(n: Int) raises -> Float64:
...
def compute(n: PythonObject) raises -> PythonObject:
    var ni = Int(py=n)
    return PythonObject(run_nbody(ni))


def bench(n: PythonObject) raises -> PythonObject:
    var ni = Int(py=n)
    var time = Python.import_module("time")
    var t0 = time.perf_counter()
    var energy = run_nbody(ni)
    var t1 = time.perf_counter()
    return Python.tuple(energy, t1 - t0)


@fieldwise_init
struct Sim(Defaultable, Movable, Writable):
    var bodies: Optional[Pointer[Body, MutUntrackedOrigin]]
    ...
    def __deinit__(deinit self):
        if self.bodies is not None:
            self.bodies.value().free()

    @staticmethod
    def py_init(out self: Sim, args: PythonObject, kwargs: PythonObject) raises:
        self = Sim()
        self.bodies = create_bodies()
        offset_momentum(self.bodies.value(), 5)

    @staticmethod
    def advance(self_ptr: Pointer[Sim, MutAnyOrigin], steps: PythonObject) raises -> PythonObject:
        var n = Int(py=steps)
        for _ in range(n):
            advance(0.01, self_ptr[].bodies.value(), 5)
        return Python.none()

    @staticmethod
    def run(self_ptr: Pointer[Sim, MutAnyOrigin], steps: PythonObject) raises -> PythonObject:
        var n = Int(py=steps)
        _ = report_energy(self_ptr[].bodies.value(), 5)
        for _ in range(n):
            advance(0.01, self_ptr[].bodies.value(), 5)
        var energy = report_energy(self_ptr[].bodies.value(), 5)
        return PythonObject(energy)

    @staticmethod
    def energy(self_ptr: Pointer[Sim, MutAnyOrigin]) raises -> PythonObject:
        return PythonObject(report_energy(self_ptr[].bodies.value(), 5))


@export
def PyInit_nbody() abi("C") -> PythonObject:
    try:
        var m = PythonModuleBuilder("nbody")
        m.def_function[compute]("compute")
        m.def_function[bench]("bench")
        _ = (
            m.add_type[Sim]("Sim")
            .def_py_init[Sim.py_init]()
            .def_method[Sim.advance]("advance")
            .def_method[Sim.run]("run")
            .def_method[Sim.energy]("energy")
        )
        return m.finalize()
    except e:
